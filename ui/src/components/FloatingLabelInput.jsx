...
const FloatingLabelInput = ({ label, onchange, arg_name = undefined, value = "", className = "", }) => {
    arg_name = arg_name == undefined ? label : arg_name
    return (
        <div className={`relative ${className} break-normal`}>
            <input type="text" className="block rounded-md px-2.5 pb-2.5 pt-5 w-full text-md peer focus:outline-none bg-white" placeholder=" " value={value}
                onChange={(e) => onchange(e.target.value, arg_name)} />
