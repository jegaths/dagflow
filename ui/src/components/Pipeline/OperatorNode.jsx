    ...
    <div className="operator">
      ...
      <div className={`${node_style} px-3 py-2 rounded-md shadow-lg`}>
        <div className={`${header_style} text-xl flex items-center gap-2`}>
          <FiGitCommit />
          <div className="w-[15rem] break-all">{pipelineData.operators[data] != undefined && pipelineData.operators[data]["args"]["task_id"]}</div>
        </div>
        ...
        {pipelineData.operators[data] != undefined && <div className="text-[0.7rem] font-light break-normal w-[15rem]">{pipelineData.operators[data]["description"]}</div>}
      </div>
